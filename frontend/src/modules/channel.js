...
export const createChannelController = ({ api, refreshDashboard }) => {
  ...
  const mount = (root) => {
    ...
    const pageSizeControl = root.querySelector('[data-channel-page-size]');
    const exportLink = root.querySelector('[data-channel-video-export]');
    let loadedVideos = [];
    let selectedVideoId = null;
    const comparedVideoIds = new Set();
    let contentLoading = false;
    let currentPage = 1; let totalPages = 1; let totalVideos = 0; let pageSize = 12;
    if (exportLink && typeof api.channelVideoExportUrl === 'function') exportLink.href = api.channelVideoExportUrl();
    ...
    const renderComparison = () => {
      ...
      if (selected.length === 2) {
        ...
        const table = document.createElement('table'); table.className = 'channel-comparison-table'; table.setAttribute('aria-label', 'Comparação de métricas dos vídeos selecionados');
    ...
    const changePageSize = () => { const next = Number(pageSizeControl?.value); if ([12, 24, 50].includes(next) && next !== pageSize) { pageSize = next; currentPage = 1; loadContent(); } };
    const sync = async () => {
    ...
    previousPage?.addEventListener('click', goPrevious); nextPage?.addEventListener('click', goNext); pageSizeControl?.addEventListener('change', changePageSize);
    cleanup = () => { button?.removeEventListener('click', sync); search?.removeEventListener('input', onFilter); format?.removeEventListener('change', onFilter); sort?.removeEventListener('change', onFilter); evidence?.removeEventListener('change', onFilter); resetFilters?.removeEventListener('click', reset); refreshContent?.removeEventListener('click', loadContent); previousPage?.removeEventListener('click', goPrevious); nextPage?.removeEventListener('click', goNext); pageSizeControl?.removeEventListener('change', changePageSize); };
