...
const render = () => createPanel({ eyebrow: 'Produção local', title: 'Renderizar corte', className: 'renders-panel', body: html`
  ...
  <p class="research-disclaimer">Imagem inteira preserva as bordas e preenche o espaço restante com preto. Recortar o centro ocupa a tela vertical e pode remover informações laterais. Saída em 720 × 1280; sem legendas sobrepostas ou publicação automática.</p>
...
export const createRendersController = ({ api, schedule = setTimeout, unschedule = clearTimeout }) => {
  ...
  const mount = (root, context = {}) => {
    ...
    const stopVideo = () => { if (video) { video.pause?.(); video.removeAttribute('src'); video.load?.(); video = null; } };
    ...
    const showJob = (job) => {
      ...
      if (job.status === 'SUCCEEDED' && job.previewUrl) {
        ...
        video = el('video'); video.controls = true; video.preload = 'metadata'; video.className = 'render-preview'; video.src = api.renderPreviewUrl(job.id); video.setAttribute('aria-label', 'Prévia do corte renderizado');
