  ...
  const mount = (root) => {
    ...
    const renderDetail = (row) => {
      ...
      for (const step of row.steps ?? []) { const card = document.createElement('section'); card.className = 'planning-item'; card.dataset.productionStep = step.key; card.append(el('strong', `${step.position}. ${step.label}`), el('span', `${step.state} - ${step.mode}`), el('small', step.capability ? `Capacidade: ${step.capability}` : step.mode === 'MANUAL' ? 'Etapa manual' : ''));
        ...
        if (['IN_PROGRESS', 'WAITING_USER'].includes(step.state) && step.key !== 'REVIEW') controls.append(action('Concluir', { productionAction: 'complete', stepKey: step.key }, true));
    ...
    const click = async (event) => { const openTarget = event.target.closest?.('[data-production-open]'); if (openTarget) return open(openTarget.dataset.productionOpen); const packagingOpen = event.target.closest?.('[data-production-packaging-open]'); if (packagingOpen) { location.hash = '#/packaging'; return; } const target = event.target.closest?.('[data-production-action]'); if (!target || pending || !activeId) return; pending = true;
