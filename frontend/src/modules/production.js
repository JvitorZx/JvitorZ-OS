  ...
  const mount = (root, context = {}) => {
    ...
    const renderDetail = (row) => {
      ...
      for (const step of row.steps ?? []) { const card = document.createElement('section'); card.className = 'planning-item'; card.dataset.productionStep = step.key; card.append(el('strong', `${step.position}. ${step.label}`), el('span', `${step.state} - ${step.mode}`), el('small', step.capability ? `Capacidade: ${step.capability}` : step.mode === 'MANUAL' ? 'Etapa manual' : ''));
        ...
        if (step.key === 'SHORTS') controls.append(action('Abrir Shorts', { productionShortsOpen: row.id }, true));
        ...
        if (['IN_PROGRESS', 'WAITING_USER'].includes(step.state) && !['REVIEW', 'CHAPTERS', 'SHORTS'].includes(step.key)) controls.append(action('Concluir', { productionAction: 'complete', stepKey: step.key }, true));
    ...
    const click = async (event) => { const openTarget = event.target.closest?.('[data-production-open]'); if (openTarget) return open(openTarget.dataset.productionOpen); const packagingOpen = event.target.closest?.('[data-production-packaging-open]'); if (packagingOpen) { location.hash = '#/packaging'; return; } const shortsOpen = event.target.closest?.('[data-production-shorts-open]'); if (shortsOpen) { location.hash = `#/shorts/${encodeURIComponent(shortsOpen.dataset.productionShortsOpen)}`; return; } const chaptersOpen = event.target.closest?.('[data-production-chapters-open]'); if (chaptersOpen) { location.hash = '#/chapters'; return; } const target = event.target.closest?.('[data-production-action]'); if (!target || pending || !activeId) return; pending = true;
    ...
    form.addEventListener('submit', submit); list.addEventListener('click', click); detail.addEventListener('click', click); cleanup = () => { form.removeEventListener('submit', submit); list.removeEventListener('click', click); detail.removeEventListener('click', click); }; load(); if (context.route?.subpath) { try { open(decodeURIComponent(context.route.subpath)); } catch { message('Identificador de produção inválido.', 'error'); } }
  };
  const unmount = () => { cleanup(); cleanup = () => {}; mounted = null; generation += 1; listRequest += 1; detailRequest += 1; pending = false; activeId = null; };
  return { mount, unmount };
};

export const productionModule = { id: 'production', route: '/production', allowSubroutes: true, label: 'Producao', icon: 'planning', fullscreen: true, pageTitle: 'Content Production Pipeline', pageEyebrow: 'Do plano ao resultado', render: renderProduction, createController: createProductionController };
