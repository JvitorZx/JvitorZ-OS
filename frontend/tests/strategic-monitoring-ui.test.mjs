...
const createDom = () => {
  ...
  for (const selector of ['[data-monitoring-severity]', '[data-monitoring-state]', '[data-monitoring-type]', '[data-monitoring-evaluate]', '[data-monitoring-feedback]', '[data-monitoring-list]', '[data-monitoring-detail]']) panel.map.set(selector, new FakeElement());
...
test('central API client exposes safe monitoring contracts and filters', async () => {
  ...
  await api.getStrategicSignal('signal/1'); await api.evaluateStrategicMonitoring();
  ...
  assert.deepEqual(calls.map(([url]) => url), [
    ...
    'http://localhost:3000/api/monitoring/signals/signal%2F1', 'http://localhost:3000/api/monitoring/evaluate',
  ...
  assert.equal(calls[2][1].body, '{}'); assert.equal(calls[3][1].body, '{"reason":"Revisado"}');
...
test('monitoring is a lifecycle-enabled fullscreen workspace with accessible local feedback', () => {
  ...
  assert.match(markup, /data-monitoring-list/); assert.match(markup, /aria-live="polite"/); assert.doesNotMatch(markup, /statePanel/);
});
...
test('manual evaluation is single-flight and refreshes persisted signals', async () => {
  ...
  createMonitoringController({ api: { listStrategicSignals: async () => { loads += 1; return []; }, evaluateStrategicMonitoring: async () => { evaluations += 1; return pending.promise; } } }).mount(dom.root); await flush();
  ...
  pending.resolve({ unchanged: false }); await Promise.all([first, second]); assert.equal(loads, 2);
});
