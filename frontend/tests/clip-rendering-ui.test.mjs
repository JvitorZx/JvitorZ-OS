...
test('caption preview attaches one VTT track to the selected output with CORS enabled', async () => {
  const page = await start({ ...defaults(), listRenderJobs: async () => [job('SUCCEEDED')], getClipCaptions: async () => ({ available: true, cueCount: 1, cues: [{ startMs: 0, endMs: 2000, text: 'Olá' }], formats: ['vtt'] }), clipCaptionsDownloadUrl: (id) => `http://localhost:3000/api/renders/jobs/${id}/captions/vtt` });
  await page.panel.dispatch('click', action('open', 'j1')); await page.panel.dispatch('click', action('captions', 'j1')); const button = nodes(page.get('detail')).find((node) => node.tagName === 'BUTTON' && node.textContent === 'Mostrar legendas na prévia'); await button.dispatch('click'); await button.dispatch('click'); const video = nodes(page.get('detail')).find((node) => node.tagName === 'VIDEO'); const tracks = video.children.filter((node) => node.tagName === 'TRACK'); assert.equal(video.crossOrigin, 'anonymous'); assert.equal(tracks.length, 1); assert.equal(tracks[0].src, 'http://localhost:3000/api/renders/jobs/j1/captions/vtt'); page.controller.unmount(); await button.dispatch('click'); assert.equal(tracks.length, 1);
});
