...
const createDom = () => {
  ...
  const selectors = ['[data-automation-form]', '[data-automation-id]', '[data-automation-name]', '[data-automation-template]',
    ...
    '[data-automation-list]', '[data-automation-runs]'];
...
test('controller loads real automations once and renders untrusted names as text', async () => {
  let calls = 0; const api = { listAutomations: async () => { calls += 1; return [item]; } };
...
test('form creates a manual automation through the central client payload', async () => {
  let received; const api = { listAutomations: async () => [], createAutomation: async (input) => { received = input; return item; } };
...
test('Run Now ignores a repeated click while the request is active', async () => {
  const pending = deferred(); let calls = 0; const api = { listAutomations: async () => [item],
...
test('late list response after unmount cannot replace the detached workspace', async () => {
  const pending = deferred(); const dom = createDom(); const controller = createAutomationsController({ api: { listAutomations: async () => pending.promise } });
