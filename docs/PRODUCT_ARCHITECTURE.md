...
Entre as Sprints 81-98, `ChannelContentService` ganhou paginação após deduplicação, coortes factuais de cobertura por formato e exportação CSV segura. O endpoint legado de lista foi preservado. A workspace permite selecionar até dois vídeos, informa incompatibilidade de formato ou momento de coleta, descreve somente direção observada e oferece paginação, tamanho de página e download local. Nenhuma dessas leituras e transformações declara vencedor, causalidade ou previsão.
